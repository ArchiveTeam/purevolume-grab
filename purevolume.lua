...
allowed = function(url, parenturl)
  if string.match(url, "'+")
      or string.match(url, "[<>\\%*%$;%^%[%],%(%)]")
      or string.match(url, "//$")
      or not string.match(url, "https?://[^/]*purevolume[^/]*%.com/") then
    return false
  end
  ...
  if string.match(url, "https?://g%.purevolumecdn%.com/cdnImages/") then
    return true
  end
  ...
  if status_code >= 500 or
    ...
    if tries >= 5 then
      ...
      if (allowed(url["url"], nil) or status_code == 500)
          and not string.match(url["url"], "https?://g%.purevolumecdn%.com/") then
        return wget.actions.ABORT
      else
